...
public class UserServiceDBSQL implements UserService {
    ...
    public void addUser(User user) {
        String query = String.format("insert into %s.user (email,password,role,name) values (?,?,?,?)", this.schema);
    ...
    public void removeUser(int user) {
        String query = String.format("DELETE FROM %s.user WHERE id = ?", schema);
    ...
    public ArrayList<User> getAllDistributors() {
        ArrayList<User> users = new ArrayList<>();
        String query = String.format("SELECT * FROM %s.user WHERE role = 'VERDELER' order by id" , schema);
        try {
            PreparedStatement preparedStatement = getConnection().prepareStatement(query);
            ResultSet result = preparedStatement.executeQuery();
            while (result.next()) {
    ...
    public ArrayList<User> getAllProviders() {
        ArrayList<User> users = new ArrayList<>();
        String query = String.format("SELECT * FROM %s.user WHERE role = 'AANBIEDER' order by id" , schema);
        try {
            PreparedStatement preparedStatement = getConnection().prepareStatement(query);
            ResultSet result = preparedStatement.executeQuery();
            while (result.next()) {
    ...
    public void editUser(int id, String email, Role role, String name) {
        String query = String.format("UPDATE %s.user SET name = ?, email = ?, role = ? WHERE id = ?", schema);
        try {
            PreparedStatement preparedStatement = getConnection().prepareStatement(query);
